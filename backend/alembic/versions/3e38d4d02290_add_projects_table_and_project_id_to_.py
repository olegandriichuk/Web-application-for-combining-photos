...
def upgrade() -> None:
    ...
    # Add project_id to photos — native PostgreSQL ALTER TABLE
    op.add_column('photos', sa.Column('project_id', sa.String(length=36), nullable=True))
    op.create_index('ix_photos_project_id', 'photos', ['project_id'], unique=False)
    op.create_foreign_key('fk_photos_project_id', 'photos', 'projects', ['project_id'], ['id'], ondelete='CASCADE')

    # SQLite: batch_alter_table was required because SQLite cannot ALTER TABLE directly
...
def downgrade() -> None:
    op.drop_constraint('fk_photos_project_id', 'photos', type_='foreignkey')
    ...
    # SQLite: batch_alter_table downgrade
    # with op.batch_alter_table('photos', schema=None) as batch_op:
